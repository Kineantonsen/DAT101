...
export class TTile extends libSprite.TSpriteButton {
  ...
  onMouseDown(aEvent) {
    ...
    this.index = 1;
    gameProps.ScoreBoard.Smiley.index = 1;
  }
  ...
  onMouseUp(aEvent) {
    ...
    if (this.#isMine) {
      this.index = 4;
      gameProps.ScoreBoard.Smiley.index = 2;
      setGameOver();
    } else {
      this.index = 2;
      this.disable = true;
      gameProps.ScoreBoard.Smiley.index = 0;
      if (this.#mineInfo === 0) {
    ...
    this.disable = true;
    //Sjekk om spillet er slutt. Hint: bruk forEachTile
    ...
    if (tilesLeft === gameLevel.Mines){
      gameProps.ScoreBoard.Smiley.index = 4;
      setGameOver();
    }
  }
